        ...
        [ describe "DeclarationOrder"
            [test "preserves declaration order" (fn _ =>
               let
                 ...
                 val (ordered, seedOpt) =
                   applyExecutionOrder Configuration.DeclarationOrder runners
                 val labels = getLabels ordered
                 val expected =
                   ["suite.zebra", "suite.apple", "suite.mango", "suite.banana"]
               in
                 Expect.all
        ...
        , describe "AlphabeticalOrder"
            [ test "sorts tests alphabetically by label" (fn _ =>
                let
                  val runners = makeTestRunners ()
                  val (ordered, seedOpt) =
                    applyExecutionOrder Configuration.AlphabeticalOrder runners
                  val labels = getLabels ordered
                  val expected =
                    [ "suite.apple"
                    , "suite.banana"
            ...
            , test "handles nested describes" (fn _ =>
                let
                  val tests = describe "root"
                    [ describe "z-group" [test "first" expectPass]
                    , describe "a-group" [test "second" expectPass]
                    ]
                  val runners =
                    case fromTest tests of
                      Plain {runners, ...} => runners
        ...
        , describe "RandomOrder"
            [ test "shuffles tests" (fn _ =>
                let
                  val runners = makeTestRunners ()
                  val (ordered, seedOpt) =
                    applyExecutionOrder (Configuration.RandomOrder NONE) runners
                ...
                in
                  Expect.all
                    ...
                    , Expect.equal Int.compare (List.length originalLabels)
                        (List.length shuffledLabels)
                    ]
                end)
            ...
            , test "same seed produces same order" (fn _ =>
                let
                  ...
                  val runners = makeTestRunners ()
                  val (ordered1, seed1) =
                    applyExecutionOrder (Configuration.RandomOrder (SOME seed))
                      runners
            ...
            , test "different seeds produce different orders" (fn _ =>
                let
                  ...
                  val (ordered1, _) =
                    applyExecutionOrder (Configuration.RandomOrder (SOME 111))
                      runners1
                  val labels1 = getLabels ordered1

                  val runners2 = makeTestRunners ()
                  val (ordered2, _) =
                    applyExecutionOrder (Configuration.RandomOrder (SOME 999))
                      runners2
                  val labels2 = getLabels ordered2
                in
